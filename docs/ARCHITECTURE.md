...
mode), persists the stream to disk, and reports your most repeated keystrokes
and commands. It is written in Lua for Neovim 0.11+ and LazyVim, with no
required dependencies. The Vim 9 version is on the `legacy-vim9` branch.
...
`build(events, { limit, since })` returns
...
Key pairs are counted per session, so no pair spans two editor runs.
...
The report opens in a centred float that `q` closes; section titles are
highlighted.

## Commands and options

`:KeyHabits [report [days] | start | stop | clear[!]]`, completed; with no
argument, the report. `require("keyhabits")` exposes the same as functions,
plus `is_recording()` and `log_file()`. Options, with defaults, are in
`config.lua` and `:help keyhabits`.
